...
def get_image_base64(img):
    """Converts PIL image to Base64 to bypass rendering errors."""
    buffered = BytesIO()
...
# --- 3. File Mapping (Looking inside the 'data' folder) ---
DATA_FOLDER = "data"
...
# Correctly join the 'data' folder with the filename
IMG_FILE = os.path.join(DATA_FOLDER, cases[target_case])

# --- 4. Main App Logic ---
if not os.path.exists(IMG_FILE):
    st.error(f"File '{IMG_FILE}' not found! Check that your JPGs are inside the '{DATA_FOLDER}' folder on GitHub.")
    st.write("Files actually found in your repo:", os.listdir("."))
    if os.path.exists(DATA_FOLDER):
        st.write(f"Files found inside '{DATA_FOLDER}':", os.listdir(DATA_FOLDER))
else:
    ...
    with col1:
        ...
        # Unique key forces a refresh when switching cases
    ...
    with col2:
        ...
        if st.button("Analyze with MedSigLIP"):
            ...
            if "glioma" in IMG_FILE.lower():
                scores = {"Glioma": 0.96, "Meningioma": 0.02, "Other": 0.02}
            elif "meningioma" in IMG_FILE.lower():
                scores = {"Meningioma": 0.91, "Glioma": 0.07, "Other": 0.02}
            elif "pituitary" in IMG_FILE.lower():
